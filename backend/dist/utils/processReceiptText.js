...
exports.processReceiptItemLines = exports.processReceiptText = void 0;
const dataCleaning_1 = require("./dataCleaning");
const mathFunc_1 = require("./logicFunc/mathFunc");
const regexParsing_1 = require("./regexParsing");
// Setup interface for receipt
function processReceiptText(text, uid) {
    ...
    updatedText = (0, dataCleaning_1.cleanNoncaptureLines)(updatedText);
    // Find and remove COSTCO and WHOLESALE from the text
    receipt.supplier = (0, regexParsing_1.extractSupplierName)(updatedText, /\s*COSTCO/);
    updatedText = (0, dataCleaning_1.cleanDataWithRegex)(updatedText, /\s*COSTCO/);
    // Extracting "WHOLESALE" and appending it to the supplier string
    const wholesalePart = (0, regexParsing_1.extractSupplierName)(updatedText, /\s*WHOLESALE/);
    if (wholesalePart) {
        receipt.supplier += " " + wholesalePart;
    }
    updatedText = (0, dataCleaning_1.cleanDataWithRegex)(updatedText, /\s*WHOLESALE/);
    // Barcode number extraction (should be used for returns in future)
    receipt.barcodeNumber = (0, regexParsing_1.extractBarcodeNumber)(updatedText, /\b(\d{23})\b/);
    updatedText = (0, dataCleaning_1.cleanDataWithRegex)(updatedText, /\b(\d{23})\b/);
    // Extract store address, two checks in case first fails
    receipt.storeAddress = (0, regexParsing_1.extractStoreAddress)(updatedText, /(?:#\d{1,4}\s+)([\d\w\s.]+,\s+[A-Z]{2}\s+\d{5})/);
    updatedText = updatedText.replace(/(?<=#\d{1,4}\s+)([\d\w\s.]+,\s+[A-Z]{2}\s+\d{5})/, "");
    if (receipt.storeAddress == "") {
        receipt.storeAddress = (0, regexParsing_1.extractStoreAddressTwo)(updatedText, /(\d{1,}) [a-zA-Z0-9\s]+(?:Avenue|Lane|Road|Boulevard|Drive|Street|Suite|Ave|Dr|Rd|Blvd|Ln|St|Ste)?,?\s+[a-zA-Z]+,?\s+[A-Z]{2} [0-9]{5,6}/);
    }
    updatedText = updatedText.replace(/(\d{1,}) [a-zA-Z0-9\s]+(?:Avenue|Lane|Road|Boulevard|Drive|Street|Suite|Ave|Dr|Rd|Blvd|Ln|St|Ste)?,?\s+[a-zA-Z]+,?\s+[A-Z]{2} [0-9]{5,6}/, "");
    // Extract store name
    receipt.storeName = (0, regexParsing_1.extractStoreName)(updatedText, /([a-zA-Z ]+)(?=\s#\d+)/);
    updatedText = updatedText.replace(/([a-zA-Z ]+)(?=\s#\d+)/, "");
    // Extract store number
    receipt.storeNumber = (0, regexParsing_1.extractStoreNumber)(updatedText, /#\s*(\d+)/);
    updatedText = updatedText.replace(/#\s*(\d+)/, "");
    updatedText = updatedText.replace(/(?:#\d{1,4}\s+)([\d\w\s.]+,\s+[A-Z]{2}\s+\d{5})/, "");
    const memberInfo = (0, regexParsing_1.extractMemberNumber)(updatedText, /\s?([A-Za-z0-9]{2})\s+Member\s+((111|9|8)\d{6,})\b/);
    receipt.memberNumber = memberInfo.memberNumber;
    receipt.dayCode = memberInfo.dayCode;
    updatedText = updatedText.replace(/\s?([A-Za-z0-9]{2})\s+Member\s+((111|9|8)\d{6,})\b/, "");
    // Extract store, terminal, transaction, and operator numbers. Only change store number if blank from first check.
    const numberSequence = (0, regexParsing_1.extractStoreTrmTrnOp)(updatedText, /(\d{1,2}:\d{2}.*?)(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/s);
    receipt.storeNumber == ""
    ...
    updatedText = (0, dataCleaning_1.cleanDataWithRegex)(updatedText, /(?:\d{1,2}:\d{2}.*?)(\d+\s+\d+\s+\d+\s+\d+)/g);
    // Extract date of purchase
    receipt.dateOfPurchase = (0, regexParsing_1.extractDataWithRegex)(updatedText, /(\d{2}\/\d{2}\/\d{4})/g, "");
    updatedText = (0, dataCleaning_1.cleanDataWithRegex)(updatedText, /(\d{2}\/\d{2}\/\d{4})/g);
    // Extract time of purchase
    receipt.timeOfPurchase = (0, regexParsing_1.extractDataWithRegex)(updatedText, /(\d{2}:\d{2})/g, "");
    updatedText = updatedText.replace(/(\d{2}:\d{2})/g, "");
    // Extract number of items
    receipt.numberOfItems = (0, regexParsing_1.extractItemsSold)(updatedText);
    // 
    // Extract total amount of purchase
    const totalAmountRegex = /AMOUNT: *\$(\d+\.\d{2})/;
    receipt.totalAmount = (0, regexParsing_1.extractTotalAmount)(updatedText, totalAmountRegex);
    updatedText = updatedText.replace("AMOUNT", "");
    // Extract multiplier lines (lines with qty and price)
    const extractionResult = (0, regexParsing_1.extractMultiplierLines)(updatedText);
    let multiplierLines = extractionResult.multiplierLines;
    updatedText = extractionResult.updatedText;
    // Extract an array of coupon amounts
    const couponExtractionResult = (0, regexParsing_1.extractCouponAmounts)(updatedText);
    ...
    let dollarAmounts = (0, regexParsing_1.extractDollarAmounts)(updatedText);
    ...
    const { taxAmount, execRebateAmount, subtotal } = (0, mathFunc_1.findTaxAndRebateAmounts)(receipt.totalAmount, dollarAmounts, includeExecRebate);
    ...
    dollarAmounts = dollarAmounts.filter((amount) => Math.abs(amount - receipt.taxAmount) >= epsilon &&
...
exports.processReceiptText = processReceiptText;
// Utility function to create a new line item
function createLineItem(userId, receiptId, itemNumber, itemDesc) {
    return {
        userId,
        receiptId,
        itemNumber,
...
// The dollar amounts array
// The number of items on the receipt
function processReceiptItemLines(uid, updatedText, receiptId, multiplierLines, couponAmounts, dollarAmounts, numberOfItemsOnReceipt) {
    let itemLines = [];
    let remainingText = updatedText;
    // Helper function to process line items based on a given regex
    function processLineItems(regex, text) {
        let regexMatch;
        while ((regexMatch = regex.exec(text)) !== null) {
            if (!/^\d{3}/.test(regexMatch[2]) &&
    ...
    if (itemLines.length < numberOfItemsOnReceipt) {
        ...
        while ((lineItemRegexTwoMatch = lineItemRegexTwo.exec(updatedText)) !== null) {
            if (!/^\d{3}/.test(lineItemRegexTwoMatch[2]) &&
    ...
    // usage
    numberOfItemsOnReceipt = (0, mathFunc_1.processMultiplierLines)(multiplierLines, itemLines);
    ...
    itemLines = (0, mathFunc_1.assignCouponsToItems)(itemLines, couponAmounts);
    const result = (0, mathFunc_1.processDollarAmounts)(dollarAmounts, itemLines);
    itemLines = result.itemLines;
    return { updatedItemLines: itemLines };
}
exports.processReceiptItemLines = processReceiptItemLines;
